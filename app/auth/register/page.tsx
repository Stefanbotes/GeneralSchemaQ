...
function RegisterForm() {
  ...
  const onSubmit = async (data: RegisterFormData) => {
    try {
      ...
      // ✅ PRESERVE callbackUrl when sending to verify-email page
      const verifyUrl = `/auth/verify-email?email=${encodeURIComponent(
        email
      )}&callbackUrl=${encodeURIComponent(callbackUrl)}`;

      router.push(verifyUrl);
      // If you ever want to skip verify & go straight to login:
      ...
      <div className="w-full max-w-md">
        ...
        <Card className="bg-white shadow-xl">
          <CardHeader className="space-y-4 text-center">
            ...
            <CardTitle className="text-2xl font-bold bg-gradient-to-r from-primary-600 to-[#fcd0b1]-600 bg-clip-text text-transparent">
              Create Account
            </CardTitle>
            <CardDescription>
              Join Inner PersonaAssessment to discover your Inner Persona
              potential
            </CardDescription>
          </CardHeader>
          ...
          <CardContent>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <Button
                type="submit"
                className="w-full bg-gradient-to-r from-primary-600 to-[#fcd0b1]-600 hover:from-primary-700 hover:to-[#fcd0b1]-700"
                disabled={isLoading}
              >
