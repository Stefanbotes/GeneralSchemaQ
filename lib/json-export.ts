...
// Updated to support 108-item LASBI with proper CUID format
...
// Maps questionnaire numeric IDs (1.1.1) to Studio CUID format
// 108 items total: 18 schemas × 6 items each

const NUMERIC_TO_CUID_MAP: Record<string, string> = {
  // Section 1: Disconnection & Rejection (1.1 - 1.5)
...
function mapItemsToCUID(items: StudioItem[]): StudioItem[] {
  return items.map(item => {
    ...
    if (!cuid) {
      throw new Error(
        `No CUID mapping found for item "${item.id}". ` +
        `Valid numeric IDs are 1.1.1 through 5.4.6 (108 items total).`
      );
    }
    return {
      id: cuid,
  ...
  const payloadWithoutChecksum = {
    ...
    assessment: {
      ...
      instrument: {
        ...
        items: mappedItems, // *** Use mapped CUIDs ***
      },
    },
    provenance: {
      sourceApp,
...
/* -------------------------- Utility: Get Mapping Info ------------------- */

// Export the mapping for reference/debugging
export function getCUIDMapping() {
  return NUMERIC_TO_CUID_MAP;
}

// Check if a numeric ID has a mapping
export function hasCUIDMapping(numericId: string): boolean {
  return numericId in NUMERIC_TO_CUID_MAP;
}

// Get CUID for a numeric ID (returns null if not found)
